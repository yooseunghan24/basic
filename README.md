# 웹 퍼블리셔란?
- 웹 디자이너로부터 받은 시안을 토대로 웹 페이지를 만드는 직군. 이때 웹 표준, 웹 접근성, 크로스브라우징, 시맨틱 마크업 원칙에 맞추어 코딩한다.
...
- HTTP에 보안을 추가한 것. SSL(보안 소켓 계층)을 사용
- SSL은 서버와 브라우저 사이에 안전하게 암호화된 연결을 만들 수 있게 도와주고, 서버 브라우저가 민감한 정보를 주고받을 때 이것이 도난당하는 것을 막아줌.
- 정리글: http://blog.wishket.com/http-vs-https-%EC%B0%A8%EC%9D%B4-%EC%95%8C%EB%A9%B4-%EC%82%AC%EC%9D%B4%ED%8A%B8%EC%9D%98-%EB%A0%88%EB%B2%A8%EC%9D%B4-%EB%B3%B4%EC%9D%B8%EB%8B%A4/
...
- typescript가 기본임.
## React(라이브러리)
- Virtual DOM: 인터렉션이 발생하게 되면 브라우저의 DOM에 접근해서 변화를 반영하는 것이 아니라 Virtual DOM에 한 번 렌더링하고, 기존의 DOM과 비교하여 변화가 필요한 곳만 렌더링 함. 따라서 브라우저에서 DOM의 조작을 최소화 해줄 수 있음. 성능이 좋아짐.
...
- MVVM 패턴: 마크업언어나 GUI 코드를 백엔드 로직과 분리하여 개발하는 패턴
...
# 크로스브라우징
- **어떤 브라우저를 이용하든 동등한 수준의 경험을 제공하는 것.**
- 한 쇼핑몰 홈페이지를 이용할 때 크롬을 이용하든 익스플로러를 이용하든 로그인, 회원가입, 상품구매 등의 기능을 제대로 이용할 수 있어야 함.
